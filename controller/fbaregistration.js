...
con.execute_proc('call InsertFBARegistration(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',fbadata,function(data) {
	if(data[0][0].SavedStatus == 0){
		...
		finpeaceFBARegistration(data[0][0].FBAID,req,res,next);
...
function finpeaceFBARegistration(FBAID,req,res,next){
	wrapper('/api/organisation/create', 'POST', {
		  "agent_name": req.body.FirstName +" "+ req.body.LastName,
		  "contact_no": "8928385797",
		  "agent_fba_id": FBAID,
		  "agent_email":"finpeace.support@magicfinmart.com",
		  "agent_pan_no": "",
		  "country": "India",
		  "state": req.body.State,
		  "city": req.body.City,
		  ...
		  "account_no": "",
		  "ifsc_no": ""
  }, function(finpeacedata) {
  		var FinpeaceLogPara = [];
  		FinpeaceLogPara.push(FBAID);
  		FinpeaceLogPara.push(finpeacedata.data.org.org_id);
  		FinpeaceLogPara.push(finpeacedata.data.org. org_coupon_code);
  		FinpeaceLogPara.push(finpeacedata.data.bank.acc_detail_id);
  		FinpeaceLogPara.push(finpeacedata.data.bank.reference_id);
  		FinpeaceLogPara.push(finpeacedata.status);
  		FinpeaceLogPara.push(finpeacedata.value);
  		con.execute_proc('call insert_log_FinpeaceLog(?,?,?,?,?,?,?)',FinpeaceLogPara,function(respdata) {
		console.log(respdata[0][0].message);
		});
  },23);
}
